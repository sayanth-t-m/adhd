...
def analyze_image_direct_for_dyslexia(image_path):
    ...
    Based on these observations, provide a concise well structured short summary of the key dyslexia indicators present.
    Then, assign a score from 0 to 100% representing the likelihood that the handwriting shows signs of dyslexia, if its above 60.
    """
    try:
...
# --------------------- Eye Tracking & Gaze Tracker Code ---------------------
...
# --------------------- Eye Tracking GUI Application ---------------------
class EyeTrackerGUI(tk.Frame):
    def __init__(self, parent):
        super().__init__(parent)
        self.pack(fill=tk.BOTH, expand=True)
        # Instead of setting the title on a Frame, use the top-level window if needed:
        # self.winfo_toplevel().title("Eye Tracking & Gaze Tracker")
        ...
        # Log output
        self.log_text = tk.Text(self, height=8, width=100)
...
# --------------------- Dyslexia Analyzer GUI Application ---------------------
class DyslexiaAnalyzerGUI(tk.Frame):
    def __init__(self, parent):
        super().__init__(parent)
        self.pack(fill=tk.BOTH, expand=True)
        # Instead of setting the title here, use the main window's title (already set in main())
        # self.winfo_toplevel().title("Direct Visual Dyslexia Analyzer")
        ...
        self.analyze_button = tk.Button(self, text="Direct Visual Analysis for Dyslexia", command=self.analyze_image, state=tk.DISABLED)
    ...
    def _process_and_analyze(self):
        try:
            ...
            self.master.after(0, self.progress.pack_forget)
            self.master.after(0, self._update_gui_with_results, analysis, probability)
        except Exception as e:
            self.master.after(0, self.progress.stop)
            self.master.after(0, self.progress.pack_forget)
            self.master.after(0, lambda: messagebox.showerror("Error", str(e)))
            self.master.after(0, lambda: self.log("Error during analysis: " + str(e)))
        finally:
...
def main():
    root = tk.Tk()
    root.title("Multi-Tool Visual Analysis Application")
    ...
    eye_tracker_frame = tk.Frame(notebook)
    dyslexia_frame = tk.Frame(notebook)
    
    # Add tabs to the notebook
    notebook.add(eye_tracker_frame, text="Eye Tracking & Gaze")
    notebook.add(dyslexia_frame, text="Dyslexia Analyzer")
    
    # Initialize each GUI inside its respective frame
    EyeTrackerGUI(eye_tracker_frame)
    DyslexiaAnalyzerGUI(dyslexia_frame)
